    ...
    */
    ...
    {   
        ...
        if(pPlayer->m_AccData.m_Vip || IsAdmin || pPlayer->Temporary.m_PassiveMode)
            ...
            SendChatTarget(ClientID, pPlayer->m_Passive ? "Passive mode activated! But you do not have access yet to passive.(Win it with events!)" : "Passive mode desactivated");
    }
    ...
    else if (str_comp_nocase(pMsg + 1, "rainbow") == 0 && (pPlayer->m_AccData.m_Vip || IsAdmin || pPlayer->m_Level.m_LeveL >= 100))
    {
        if (!pChar || !pChar->IsAlive())
            return;
        pPlayer->m_Rainbow ^= 1;
        SendChatTarget(ClientID, pPlayer->m_Rainbow ? "Rainbow activated" : "Rainbow desactivated");
    }
    else if (str_comp_nocase(pMsg + 1, "rainbowfeet") == 0 && (pPlayer->m_AccData.m_Vip || IsAdmin || pPlayer->m_Level.m_LeveL >= 50))
    {
    if (!pChar || !pChar->IsAlive())
        return;
    pPlayer->m_RainbowFeet ^= 1;
    SendChatTarget(ClientID, pPlayer->m_RainbowFeet ? "Rainbow feet activated" : "Rainbow feet desactivated");
    }
    else if (str_comp_nocase(pMsg + 1, "circle") == 0 && (pPlayer->m_AccData.m_Vip || IsAdmin || pPlayer->m_Level.m_LeveL >= 100))
    {
        pPlayer->m_EpicCircle ^= 1;
        SendChatTarget(ClientID, pPlayer->m_EpicCircle ? "Circle activated" : "Circle desactivated"); 
    ...
    else if (str_comp_nocase(pMsg + 1, "rainbowhook") == 0 && (pPlayer->m_AccData.m_Vip || IsAdmin || pPlayer->m_Level.m_LeveL >= 150))
    {
        if (!pChar || !pChar->IsAlive())
            return;

        pPlayer->m_RainbowHook ^= 1;
        SendChatTarget(ClientID, pPlayer->m_RainbowHook ? "Rainbow hook activated" : "Rainbow hook desactivated"); 
    ...
    else if(str_comp_nocase(pMsg + 1, "stars") == 0 && pPlayer->m_Level.m_LeveL >= 300)
    {
        if (!pChar || !pChar->IsAlive())
            return;

        pPlayer->m_Stars ^= true;
        SendChatTarget(ClientID, pPlayer->m_Stars ? "Stars activated" : "Stars desactivated");
    }
