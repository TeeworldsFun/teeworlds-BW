...
void CGameContext::CreateArmorEvent(vec2 Pos, int Owner)
{
	new CArmor(&m_World, Pos, Owner);
}
...
void CGameContext::SendChatTarget(int To, const char *pText, int From)
{
	CNetMsg_Sv_Chat Msg;
	Msg.m_Team = 0;
	Msg.m_ClientID = From;
...
void CGameContext::HandleFlagHunt()
{
	...
	if (s_StartFlagHunt == true)
	{
		...
		SendBroadcast("A Flag-Hunt began. Find and touch the Flag", -1);
		return;
	}
...
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int ClientID = clamp(pResult->GetInteger(0), 0, 64);
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(!pSelf->m_apPlayers[i] || i == ClientID)
			continue;
		pSelf->SendChatTarget(i, pResult->GetString(1), ClientID);
	}
}
...
int CGameContext::ProcessSpamProtection(int ClientID)
{
	...
	{
		Mute(0, &Addr, g_Config.m_SvSpamMuteDuration, Server()->ClientName(ClientID), false);
...
void CGameContext::List(int ClientID, const char* filter)
{
	...
	for (int i = 0; i < MAX_CLIENTS; i++)
	{
		if (m_apPlayers[i] && !m_apPlayers[i]->m_Invisible)
		{
			total++;
