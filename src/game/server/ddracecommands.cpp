...
void CGameContext::ConRainbowFeet(IConsole::IResult* pResult, void* pUserData) // give or remove Rainbow feet
{
	CGameContext* pSelf = (CGameContext*)pUserData;
	if (!CheckClientID(pResult->m_ClientID))
		return;
	...
	if (pSelf->m_apPlayers[Victim])
	{
		pSelf->m_apPlayers[Victim]->m_RainbowFeet ^= 1;
		char aBuf[512];
		str_format(aBuf, sizeof(aBuf), pSelf->m_apPlayers[Victim]->m_RainbowFeet ? "%s gave you rainbow feet!" : "%s removed your rainbow feet!", pSelf->Server()->ClientName(pResult->m_ClientID));
		pSelf->SendChatTarget(Victim, aBuf);
	}
}
